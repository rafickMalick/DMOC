            ...
            <x-card class="{{ $hasStepOne ? '' : 'opacity-60' }}">
                ...
                @else
                    ...
                    @php
                        ...
                        $checkoutUrl = $latestPayment?->response_data['provider']['checkout_url'] ?? null;
                    @endphp
                    @if($checkoutUrl)
                        <a href="{{ $checkoutUrl }}" target="_blank" class="mt-3 inline-block text-sm text-[#d304f4] hover:text-white">
                            Ouvrir le lien de paiement {{ strtoupper($order->payment_method ?? '') }}
                        </a>
                    @endif
                @endif
            </x-card>
